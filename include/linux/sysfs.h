...
// hhr
// 普通attribute，只能以字符串形式读写
struct attribute {
	const char		*name;
	umode_t			mode;	// 文件的权限，如0644
...
struct attribute_group {
	const char		*name;
	// 决定一个特定的attribute是否对用户空间可见，即使attribute本身定义为不可见
	// 返回值大于0表示可见，等于0表示不可见
	...
	struct attribute	**attrs;	// attribute数组，必须以NULL结尾
...
// bin attribute
struct bin_attribute {
	struct attribute	attr;	// 内嵌了一个普通attribute
	size_t			size;
	void			*private;
