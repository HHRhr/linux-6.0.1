...
#ifdef CONFIG_NUMA
	/*
		适用于NUMA（Non Uniform Memory Access Architecture）的系统，其内存空间并不是均一的，
			而是被划分成不同的node，对于不同的memory node，CPU其访问速度是不一样的
			在动态分配中断描述符的时候，应该考虑将内存分配在该CPU访问速度比较快的memory node上
	*/
	unsigned int node;
#endif
	void *handler_data;		   // 用于存储特定于中断处理程序的数据。这个成员通常被用来存储与中断处理程序相关的上下文信息或状态数据，以便在中断处理函数中使用。
	struct msi_desc *msi_desc; // 基于消息的中断Message Signaled Interrupts
#ifdef CONFIG_SMP
...
struct irq_data
{
	...
	struct irq_chip *chip; // 与chip相关的处理函数集，可以控制chip行为
 ...
 */
/*
	chip driver要提供的callback，可以控制interrupt controller行为
*/
struct irq_chip
{
	const char *name;									// 中断控制器名，用于/proc/interrupts中的显示
	unsigned int (*irq_startup)(struct irq_data *data); // 可能包含一些初始化操作
	void (*irq_shutdown)(struct irq_data *data);
	void (*irq_enable)(struct irq_data *data);
	void (*irq_disable)(struct irq_data *data);

	void (*irq_ack)(struct irq_data *data);	 //  确认中断请求。对于某些中断控制器，必须在确认后清除挂起状态才能接收新的中断。
	void (*irq_mask)(struct irq_data *data); // 屏蔽HW interrupt ID指定的中断
	void (*irq_mask_ack)(struct irq_data *data);
	void (*irq_unmask)(struct irq_data *data); // 放开HW interrupt ID指定的中断
	void (*irq_eoi)(struct irq_data *data);	   // 结束中断，某些中断控制器（如GIC）提供寄存器接口，允许CPU通知中断控制器已处理完中断。
	...
	void (*irq_bus_lock)(struct irq_data *data); // 对于连接到慢速总线（如I2C接口的IO扩展器芯片）的中断控制器，在访问这些芯片时需要锁定慢速总线。
...
#ifdef CONFIG_GENERIC_IRQ_EFFECTIVE_AFF_MASK
static inline const struct cpumask *irq_data_get_effective_affinity_mask(struct irq_data *d)
{
...
static inline const struct cpumask *irq_get_effective_affinity_mask(unsigned int irq)
{
	struct irq_data *d = irq_get_irq_data(irq);
