...
/*
	PTRS_PER_PTE 常量来表示一张页表中可以容纳的 PTE 个数

	PAGE_SHIFT 常量表示一个物理内存页的大小：2^PAGE_SHIFT，在ARM中，PAGE_SHIFT = ARM64_PAGE_SHIFT, default 12
		也可以说是 PTE（页表） 中的一个页表项 pteval_t 所指向的大小，2^12，4K，因此一个物理内存页大小为4K
*/
...
/*
	PMD_SHIFT表示 PMD 中的一个页目录项 pmdval_t 所指向的大小，2^21，2M，也就是一个PTE一级页表所能映射的大小
*/
#define PMD_SHIFT ARM64_HW_PGTABLE_LEVEL_SHIFT(2) // 21
#define PMD_SIZE (_AC(1, UL) << PMD_SHIFT)
#define PMD_MASK (~(PMD_SIZE - 1))
#define PTRS_PER_PMD (1 << (PAGE_SHIFT - 3)) // 512
#endif
...
/*
	PUD_SHIFT表示 PUD 中的一个页目录项 pudval_t 所指向的大小，2^30，1G，也就是一个PMD二级页表所能映射的大小
*/
#define PUD_SHIFT ARM64_HW_PGTABLE_LEVEL_SHIFT(1) // 30
#define PUD_SIZE (_AC(1, UL) << PUD_SHIFT)
#define PUD_MASK (~(PUD_SIZE - 1))
#define PTRS_PER_PUD (1 << (PAGE_SHIFT - 3)) // 512
#endif
 ...
 */
/*
	PGDIR_SHIFT表示 PGD 中的一个页目录项 pgdval_t 所指向的大小，2^39，512G，也就是一个PUD三级页表所能映射的大小
*/
#define PGDIR_SHIFT ARM64_HW_PGTABLE_LEVEL_SHIFT(4 - CONFIG_PGTABLE_LEVELS) // 39
#define PGDIR_SIZE (_AC(1, UL) << PGDIR_SHIFT)
#define PGDIR_MASK (~(PGDIR_SIZE - 1))
#define PTRS_PER_PGD (1 << (VA_BITS - PGDIR_SHIFT)) // 512
