				...
				int force, enum migrate_mode mode)
{
	...
	// 3.对于匿名页，为了防止迁移过程anon_vma数据结构被释放了，
	// 需要使用page_get_anon_vma增加anon_vma->refcount引用计数。
	...
	// 8.对页表进行迁移：remove_migration_ptes通过反向映射机制建立new page到进程的映射关系。
	if (page_was_mapped)
 ...
 * @private:		Private data to be passed on to get_new_page()				CMA时为NULL
 * @mode:		The migration mode that specifies the constraints for			CMA时为SYNC，表示等待页面当前的写操作完成后再进行迁移
 *			page migration, if any.
 * @reason:		The reason for page migration.									CMA时为MR_CONTIG_RANG
...
thp_subpage_migration:
	for (pass = 0; pass < 10 && (retry || thp_retry); pass++) {
		...
		// 遍历from链表，当前位置赋值给page，下一位置赋值给page2
