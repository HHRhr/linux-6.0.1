...
/*
	**
	表示一个hash节点，用来管理一组等待队列
	
	**	
	不同的futex_key经过hash计算后可能挂在同一队列上

	**
	这里采用了优先级队列，说明线程不会按照FIFO的顺序被唤醒，而是优先级高的先被唤醒
*/
struct futex_hash_bucket {
	atomic_t waiters;	// 挂起线程数
	spinlock_t lock;	// 队列的自旋锁
	struct plist_head chain;	// 优先级链表头，供属于该hash_bucket的futex_q节点挂靠
...
struct futex_q {
	...
	spinlock_t *lock_ptr;	// 指向hash bucket上的自旋锁，在操作队列前要先获取锁
	union futex_key key;	// 不同的futex_word通过hash可能挂到同一队列上，因此需要保存key来标识该节点属于哪一个futex_word
