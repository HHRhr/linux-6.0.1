...
{
	// 每个kretprobe中包含一个kprobe
	struct kretprobe *rp = container_of(p, struct kretprobe, kp);
	struct kretprobe_instance *ri;
	struct freelist_node *fn;

	// 拿到空闲的kretprobe_instance
	...
	{
		// 如果有entry_handler，但调用后返回非0值，则归还kretprobe_instance，
		// 并终止后续替换返回地址的过程
		freelist_add(&ri->freelist, &rp->freelist);
		return 0;
	}
								   ...
								   unsigned long val, void *data)
{
	...
	{
		head = &kprobe_table[i];
		hlist_for_each_entry(p, head, hlist) if (within_module_init((unsigned long)p->addr, mod) ||
												 (checkcore &&
