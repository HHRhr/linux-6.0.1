					 ...
					 struct pci_dev *dev)
{
	if (ids) {
		while (ids->vendor || ids->subvendor || ids->class_mask) {
			if (pci_match_one_device(ids, dev))		// drivers/pci/pci.h
				return ids;
			ids++;
		}
	}
						    ...
						    struct pci_dev *dev)
{
	...
	// hhr 首先检查driver_override
	// 通过修改：/sys/bus/pci/devices/<device_name>/driver_override，可以从用户空间指定设备要匹配到的驱动名
	// 这是从设备端角度修改
	// strcmp在相同时返回0
	...
	// hhr 其次比较dynamic ids列表
	// 通过写入：/sys/bus/pci/drivers/<driver_name>/new_id
	// 可以在不重新编译或重启系统的情况下，在运行时向驱动程序动态添加新的设备ID
	// 这是从驱动端角度修改
	...
	// 最后比较id_table、
	// idtable是一个包含驱动可支持设备ID的数组，遍历其中的设备ID和目标设备进行对比
	...
	// hhr 设置了driver_override，但是和已有设备id都不符合，没关系，返回一个any id
	// 保证只要driver_override==drv name相同就一定可以匹配上
...
static int pci_bus_match(struct device *dev, struct device_driver *drv)
{
	...
	if (!pci_dev->match_driver)		// hhr 设备是否需要驱动
		return 0;
