 ...
 * corresponding error when it fails.
 */
 // hhr 此函数用于填充与uevent相关的env数组，并发送uevent到用户空间
		       ...
		       char *envp_ext[])
{
	...
	// 找到最顶层的parent还是没有kset，则不允许发送消息
	// 证明：kobj要发送uevent，其顶层必须存在一个kset
	if (!top_kobj->kset) {		
	...
	if (kobj->uevent_suppress) {	// kobj开启屏蔽，忽略所有uevent
	...
	// 这里主动调用kset->uevent_ops->filter，
	// 证明：kset可以管控其下kobj是否能发送消息
	...
	// kobj应该属于一个子系统，例如总线名，如果没有，则使用kset名
	...
	if (envp_ext) {
		...
		// 3.主动调用kset->uevent_ops->uevent，填充默认属性
	...
	retval = add_uevent_var(env, "SEQNUM=%llu", ++uevent_seqnum);	// 添加序列号
	if (retval) {
